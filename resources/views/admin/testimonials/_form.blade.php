...
<section class="admin-panel admin-form-section admin-narrow-panel">
    <div class="admin-panel-head"><div><small>Verified testimonial</small><h2>{{ $editing ? 'Edit testimonial' : 'New testimonial' }}</h2></div></div>
    ...
    <div class="admin-form"><label>Quote<textarea name="quote" rows="7" required>{{ old('quote', $testimonial->quote ?? '') }}</textarea><small>Publish only if the person approved this quote.</small></label><label class="admin-check"><input type="checkbox" name="is_active" value="1" @checked(old('is_active', $testimonial->is_active ?? false))> Active on public site</label></div>
</section>
